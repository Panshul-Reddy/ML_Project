...
### 📥 **For Your Specific Case (test/ML_Project structure)**

Since you cloned into `test` folder and the project is in `ML_Project`:
...
dir   # Should show: models/, realtime_fixed_roi_subset.py, train_subset_mobilenet.py

# 3. Check if model files exist
dir models\   # Should show: asl_subset_mobilenet.h5, class_indices.json

# 4. Create virtual environment (if doesn't exist)
python -m venv venv
venv\Scripts\activate
